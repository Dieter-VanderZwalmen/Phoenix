defmodule HetProject.ProgramContext do
  @moduledoc """
  ...
  def load_users(program = %Program{}) do
    program = Repo.preload(program, :users)
    program
  end
  def load_exercises(program = %Program{}) do
    program = Repo.preload(program, :exercises)
    program
  end
  ...
  def addToUser(user,program) do
    Program.assign_program_to_user_changeset(program, user)
    |> Repo.update!()

  end
  def addExerciseToProgram(exercise,program) do
    Program.assign_exercise_to_program_changeset(program, exercise)
    |> Repo.update!()
